const app = {
    state: {
        ...
        ancv: [], checks: [], mypos: [],
        fondCaisse: 134.00,
        midiData: null // <-- Pour stocker les chiffres du midi et faire la soustraction
    },
    ...
    openRecap() {
        ...
        if (this.state.service === 'Midi') {
            ...
            // On sauvegarde les données du midi pour la soustraction du soir
            this.state.midiData = JSON.parse(JSON.stringify(f));
        } else {
            const cbTotalZ = v('cb-contact-soir') + v('cb-sans-contact-soir');
            const trTotalZ = v('tr-contact-soir') + v('tr-sans-contact-soir');
            
            // Récupération des données du midi si elles existent
            const m = this.state.midiData || { cb:0, tr:0, mypos:0, cashNet:0, ancvP:0, ancvC:0, checks:0, tva5:0, tva10:0, tva20:0 };

            f = {
                service: 'Soir', 
                // Ici on fait la soustraction (Z du Soir - Z du Midi) pour avoir le réel de la soirée
                cb: cbTotalZ - m.cb,
                tr: trTotalZ - m.tr,
                mypos: this.state.mypos.reduce((a, b) => a + b, 0), 
                cashNet: netVal, // Le cash est déjà net (compté à la main)
                ...
                tva5: v('tva5-soir') - m.tva5,
                tva10: v('tva10-soir') - m.tva10,
                tva20: v('tva20-soir') - m.tva20,
    ...
    renderFinalRecap(f) {
        ...
        let html = `
            <div class="recap-list-final">
                ...
                <p style="font-size:0.8rem; color:gray; margin-bottom:10px;">${f.service === 'Soir' ? "(Chiffres après déduction du midi)" : ""}</p>
                ${row("CB Soir", f.cb)}
                ${row("CB TR Soir", f.tr)}
                ...
                <div style="background:#f8fafc; padding:8px; border-radius:5px; margin-bottom:10px; border:1px solid #e2e8f0;">
                    <div style="font-weight:bold; margin-bottom:5px; color:#475569;">TVA (RÉEL SOIR)</div>
                    ${row("TVA 5.5%", f.tva5)}
                    ${row("TVA 10%", f.tva10)}
                    ${row("TVA 20%", f.tva20)}
                </div>
                <div class="recap-row" style="font-size:1.1rem; background:#334155; color:white; padding:8px; border-radius:5px;">
                    <span>CA TOTAL RÉEL SOIR</span><b>${caCaisse.toFixed(2)}€</b>
                </div>
